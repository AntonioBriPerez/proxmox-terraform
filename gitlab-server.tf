...
resource "proxmox_vm_qemu" "gitlab-server" {
  count = 1               # just want 1 for now, set to 0 and apply to destroy VM
  name  = "gitlab-server" #count.index starts at 0, so + 1 means this VM will be named test-vm-1 in proxmox
  ...
  ipconfig0 = "ip=192.168.18.43/24,gw=192.168.18.1"
  ciuser    = "anton"
  sshkeys   = <<EOF
...
output "ip_address_gitlab_server" {
  value = proxmox_vm_qemu.gitlab-server[0].default_ipv4_address
}

output "mac_address_gitlab_server" {
  value = proxmox_vm_qemu.gitlab-server[0].network.0.macaddr
}
