resource "proxmox_vm_qemu" "samba-server" {
  count = 1              # just want 1 for now, set to 0 and apply to destroy VM
  name  = "samba-server" #count.index starts at 0, so + 1 means this VM will be named test-vm-1 in proxmox
  ...
  ipconfig0 = "ip=192.168.18.40/24,gw=192.168.18.1"
  ciuser    = "anton"
  sshkeys   = <<EOF
