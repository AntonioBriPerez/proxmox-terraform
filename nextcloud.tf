...
resource "proxmox_vm_qemu" "nextcloud-server" {
  count = 1                  # just want 1 for now, set to 0 and apply to destroy VM
  name  = "nextcloud-server" #count.index starts at 0, so + 1 means this VM will be named test-vm-1 in proxmox
  # this now reaches out to the vars file. I could've also used this var above in the pm_api_url setting but wanted to spell it out up there. target_node is different than api_url. target_node is which node hosts the template and thus also which node will host the new VM. it can be different than the host you use to communicate with the API. the variable contains the contents "prox-1u"
  target_node = "torre-linux"
  # another variable with contents "ubuntu-2004-cloudinit-template"
  clone = "ubuntu-server-focal-30G"
  ...
  ipconfig0 = "ip=192.168.18.44/24,gw=192.168.18.1"
  ciuser    = "anton"
  sshkeys   = <<EOF
...
output "ip_address_nextcloud_server" {
  value = proxmox_vm_qemu.nextcloud-server[0].default_ipv4_address
}

output "mac_address_nextcloud_server" {
  value = proxmox_vm_qemu.nextcloud-server[0].network.0.macaddr
}
